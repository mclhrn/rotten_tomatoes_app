...
- (void)viewDidLoad
{
    ...
    gSearchQuery = [NSString stringWithFormat:@"movies.json?apikey=rnskf2r9uenzrwszs2gnr44j&q=%@", searchValue];

    gSearchQuery = [gSearchQuery stringByReplacingOccurrencesOfString:@" " withString:@"+"];
                    
    NSLog(@"Testing--- %@",searchValue);
    NSLog(@"Testing--- %@",gSearchQuery);
    
    [ApplicationDelegate.networkEngine getSearchResultsMovieList:^(NSArray *responseArray) {
